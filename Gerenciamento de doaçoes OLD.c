...
typedef struct { // Estrutura heterogênea de produtos pra casar com a parte do vinicius e doglas
    int id;
    int qtd;
    float precoUnidade;
    float desconto;
...
typedef struct { // Estrutura para doações
    char produto[50];
    int qtd;
    char data[20];
    char local[100];
...
void registroDoacao(FILE *arquiv, produto produtos[], int *totalProdutos);
...
int main() {
    ...
    do {
        ...
        switch(opcao) {
            case 1: {
                FILE *arquiv = fopen("E:\\Linguagem_C\\Projeto PIM 2\\JoaoParte\\Doacoes.txt", "a");
                if (arquiv == NULL) {
                    perror("Erro ao abrir o arquivo para registro!");
                    return 1;
                }
                registroDoacao(arquiv, produtos, &totalProdutos);
                fclose(arquiv);
                break;
            }
            case 2: {
                FILE *arquiv = fopen("E:\\Linguagem_C\\Projeto PIM 2\\JoaoParte\\Doacoes.txt", "r");
                if (arquiv == NULL) {
...
void carregarProdutos(produto produtos[], int *totalProdutos) { // Incluindo parâmetros
    *totalProdutos = 0; // Inicializa totalProdutos
    FILE *arquivo = fopen("E:\\Linguagem_C\\Projeto PIM 2\\Projeto\\bin\\Debug\\Produtos.txt", "r");
    if (arquivo == NULL) {
        printf("Arquivo não encontrado!\n");
        exit(1);
    }
    while (fscanf(arquivo, "%d,%49[^,],%d,%f,%f,%f",
                  &produtos[*totalProdutos].id, produtos[*totalProdutos].nomeProduto,
                  &produtos[*totalProdutos].qtd, &produtos[*totalProdutos].precoUnidade,
                  &produtos[*totalProdutos].desconto, &produtos[*totalProdutos].valorFinal) == 6) {
        (*totalProdutos)++;
    }
    fclose(arquivo);
}

void salvarProdutos(produto produtos[], int totalProdutos) { // Incluindo parâmetros
    ...
    for (int i = 0; i < totalProdutos; i++) {
        fprintf(arquivo, "%d,%s,%d,%.2f,%.2f,%.2f\n",
                produtos[i].id, produtos[i].nomeProduto, produtos[i].qtd,
                produtos[i].precoUnidade, produtos[i].desconto, produtos[i].valorFinal);
    }
    fclose(arquivo);
}

void registroDoacao(FILE *arquiv, produto produtos[], int *totalProdutos) { // Incluindo parâmetros
    Doacao doacao;
    bool encontrado;
...
void excluirDoacao(FILE *arquiv, produto produtos[], int *totalProdutos) {
    ...
    while (fgets(linha, sizeof(linha), arquiv) && totalDoacoes < 999) {
        sscanf(linha, "%49[^,],%d,%99[^,],%19[^\n]",
               doacoes[totalDoacoes].produto, &doacoes[totalDoacoes].qtd,
               doacoes[totalDoacoes].local, doacoes[totalDoacoes].data);
        totalDoacoes++;
    }
    fclose(arquiv);
    ...
    for (int i = 0; i < totalDoacoes; i++) {
        printf("%d. Produto: %s | Quantidade: %d | Instituição: %s | Data: %s\n",
               i + 1, doacoes[i].produto, doacoes[i].qtd, doacoes[i].local, doacoes[i].data);
    }
    ...
    arquiv = fopen("E:\\Linguagem_C\\Projeto PIM 2\\JoaoParte\\Doacoes.txt", "w");
    if (arquiv == NULL) {
